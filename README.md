...
- `receiverId`: Service Status receiver ID; when blank it uses `identifier`
- `heartbeatEnabled`: automatically report decoder health while `reader.js` is running
- `heartbeatIntervalSeconds`: health reporting interval, with a minimum of 30 seconds
...
5. Save the settings.
6. Configure the decoder client's `receiverId` with the exact same Receiver ID. Current clients automatically report while `reader.js` is running; no separate heartbeat timer is required.
