angular.module('app', ['ngRoute', 'ngResource', 'ngSanitize', 'angular-uuid', 'ui.bootstrap', 'ui.validate', 'textAngular'])
    ...
    .controller('ProfileController', ['$scope', '$routeParams', 'Api', '$uibModal', '$filter', '$location', '$timeout', '$window', function ($scope, $routeParams, Api, $uibModal, $filter, $location, $timeout, $window) {
        ...
        $scope.addPushCapcode = function() { var capcode=String($scope.push.capcode||''); if(!capcode||$scope.push.selectedCapcodes.indexOf(capcode)!==-1)return; if($scope.push.selectedCapcodes.length>=3)return pushMessage('You can follow up to three pagers.','alert-warning'); $scope.push.selectedCapcodes.push(capcode); $scope.push.capcode=''; $scope.push.capcodeSearch=''; };
        $scope.removePushCapcode = function(capcode) { $scope.push.selectedCapcodes=$scope.push.selectedCapcodes.filter(function(value){return value!==capcode;}); };
        $scope.pushCapcodeLabel = function(capcode) { var match=($scope.push.capcodes||[]).find(function(item){return String(item.address)===String(capcode);}); return match?capcode+' — '+(match.alias||'Unknown'):capcode; };
        ...
        $scope.startTwoFactor = function() { Api.TwoFactorEnrol.start({}).$promise.then(function(result) { $scope.twoFactor = {setup:true, secret:result.secret, uri:result.uri, qrCode:result.qrCode}; }, function(response) { pushMessage(response.data.error || 'Unable to start 2FA enrolment.', 'alert-danger'); }); };
