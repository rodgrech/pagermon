(function (window) {
  ...
  function phoneMapPopup() {
    return !!(window.matchMedia && window.matchMedia('(max-width: 600px)').matches);
  }

  function mapPopupOptions(maxWidth) {
    if (phoneMapPopup()) return {maxWidth: Math.min(Number(maxWidth) || 280, 280), autoPan: false};
    ...
    function draw(geojson) {
      ...
      L.geoJSON(geojson, {style: function (feature) {
        ...
        if (rating) {
          ...
          var adviceDetails = { MODERATE: 'Stay up to date and be ready to act if there is a fire.', HIGH: 'There’s a heightened risk. Be alert for fires in your area.<br>Decide what you will do if a fire starts.<br>If a fire starts, your life and property may be at risk. The safest option is to avoid bushfire risk areas.', EXTREME: 'These are dangerous fire conditions.<br>Check your bushfire plan and ensure that your property is fire ready.<br>If a fire starts, take immediate action. If you and your property are not prepared to the highest level, go to a safer location well before the fire impacts.<br>Reconsider travel through bushfire risk areas.', CATASTROPHIC: 'These are the most dangerous conditions for a fire.<br>Your life may depend on the decisions you make, even before there is a fire.<br>Stay safe by going to a safer location early in the morning or the night before.<br>Homes cannot withstand fires in these conditions. You may not be able to leave, and help may not be available.' }[today.toUpperCase()] || '';
          ...
          var detailsHtml = adviceDetails ? '<div class="cw-popup-description" style="font-size:.9em;line-height:1.45;margin:10px 0;">' + adviceDetails + '<br><a href="https://www.rfs.nsw.gov.au" target="_blank" rel="noopener">RFS website for more information</a></div>' : '';
          var fireDangerPopup = phoneMapPopup()
            ? '<strong>' + escapeHtml(name) + '</strong><br>Today: <strong>' + escapeHtml(today) + '</strong><br>Tomorrow: ' + escapeHtml(tomorrow) + '<br><strong>' + escapeHtml(advice) + '</strong>' + (rating.fireBanToday || rating.fireBanTomorrow ? '<br><strong style="color:#b52b32">TOTAL FIRE BAN DECLARED</strong>' : '') + '<br><small>Updated ' + escapeHtml(updated) + '</small><br><a href="https://www.rfs.nsw.gov.au/fire-information/fdr-and-tobans" target="_blank" rel="noopener">Official details</a>'
            : '<div class="cw-incident-popup cw-fire-danger-popup"><div class="cw-popup-heading"><i class="fa fa-fire"></i><strong>FIRE DANGER RATING</strong></div><div class="cw-popup-title">' + escapeHtml(name) + '</div>' + fireDangerGauge(today) + '<div class="cw-popup-pills"><span><small>Today</small>' + escapeHtml(today) + '</span><span><small>Tomorrow</small>' + escapeHtml(tomorrow) + '</span></div><div class="cw-popup-description cw-fire-safety-message" style="text-align:center;font-size:1.1em;padding:12px 8px;margin:10px 0;border-radius:6px;background:' + fireDangerColour(today) + ';color:' + adviceTextColour + ';"><strong>' + escapeHtml(advice) + '</strong></div>' + detailsHtml + ban + '<div class="cw-popup-agency">NSW Rural Fire Service<small>Official fire danger area rating · Updated ' + escapeHtml(updated) + '</small></div><a class="cw-popup-action" href="https://www.rfs.nsw.gov.au/fire-information/fdr-and-tobans" target="_blank" rel="noopener"><i class="fa fa-external-link-alt"></i> Official details</a></div>';
          layer.bindPopup(fireDangerPopup, mapPopupOptions(390));
        }
  ...
  function renderMap(id, incidents, rfsIncidents, aircraft, dams, gauges, algaeSites) {
    ...
    (incidents || []).forEach(function (incident) {
      ...
      var pagerContent = phoneMapPopup() ? '<strong>' + escapeHtml(incident.location || incident.agency || 'Pager incident') + '</strong><br>' + escapeHtml(incident.agency || 'Unknown agency') + '<br>' + escapeHtml(incident.messages[0] && incident.messages[0].message || '') + '<br><a href="/?view=incidents&incident=' + encodeURIComponent((incident.details && incident.details.incidentId) || incident.location || '') + '">View timeline</a>' : pagerPopup(incident);
      L.marker([incident.coordinates.lat, incident.coordinates.lng]).addTo(layerGroups.pager).bindPopup(pagerContent, mapPopupOptions(390));
    });
    (rfsIncidents || []).forEach(function (incident) {
      ...
      var officialPopup = '<div class="cw-incident-popup"><div class="cw-popup-heading"><i class="fa ' + hazard.icon + '"></i><strong>' + escapeHtml(officialAgency + ' · ' + String(incident.category || 'Official incident').toUpperCase()) + '</strong></div><div class="cw-popup-title">' + escapeHtml(incident.title) + '</div><div class="cw-popup-pills"><span><small>Agency</small>' + escapeHtml(officialAgency) + '</span><span><small>Status</small>' + escapeHtml(incident.category || 'Published') + '</span><span><small>Type</small>' + escapeHtml(hazard.kind) + '</span></div><div class="cw-popup-description">' + escapeHtml(incident.description || '') + '</div>' + npwsDetail + pagerDetail + '<div class="cw-popup-actions"><a href="/?view=incidents&incident=' + encodeURIComponent((incident.pagerMatch && incident.pagerMatch.timelineKey) || incident.title || incident.link || '') + '"><i class="fa fa-stream"></i> Timeline</a><a href="' + escapeHtml(incident.link) + '" target="_blank" rel="noopener">Official details</a></div></div>';
      var pagerCompact = incident.pagerMatch ? '<br>Matching pager traffic: ' + escapeHtml(incident.pagerMatch.pageCount) + ' page(s)' : '';
      var compactDescription = String(incident.description || '').split(/\s*·\s*/).filter(function(part) { return /^(?:ALERT LEVEL|STATUS|TYPE|UPDATED)\s*:/i.test(part); }).join(' · ');
      var officialContent = phoneMapPopup() ? '<strong>' + escapeHtml(incident.title) + '</strong><br>' + escapeHtml(officialAgency) + ' · ' + escapeHtml(incident.category || 'Published') + (compactDescription ? '<br>' + escapeHtml(compactDescription) : '') + pagerCompact + '<br><a href="' + escapeHtml(incident.link) + '" target="_blank" rel="noopener">Official details</a>' : officialPopup;
      L.marker([incident.latitude, incident.longitude], {icon: incidentIcon, zIndexOffset: 450}).addTo(layerGroups.rfs).bindPopup(officialContent, mapPopupOptions(390));
    });
    ...
    (aircraft || []).forEach(function (plane) {
      ...
      var operator = plane.aircraftOperator || plane.fireOperator || '';
      var fireInfo = plane.fireAircraft ? '<div class="cw-popup-description"><strong>Fire aviation asset</strong>' + (plane.fireRole ? '<br>Role: ' + escapeHtml(plane.fireRole) : '') + '<br><small>Matched using ' + escapeHtml(plane.fireMatchSource || 'aircraft identity') + '</small></div>' : '';
      var aircraftPopup = '<div class="cw-incident-popup cw-aircraft-popup"><div class="cw-popup-heading"><i class="fa fa-plane"></i><strong>' + (plane.fireAircraft ? 'FIRE AVIATION' : 'AIRCRAFT') + '</strong></div><div class="cw-popup-title">' + escapeHtml(label) + '</div><div class="cw-popup-pills"><span><small>Registration</small>' + escapeHtml(plane.registration || 'Unknown') + '</span><span><small>ICAO</small>' + escapeHtml(plane.hex || 'Unknown') + '</span>' + (plane.aircraftType ? '<span><small>Type</small>' + escapeHtml(plane.aircraftType) + '</span>' : '') + '</div><div class="cw-popup-description"><strong>' + escapeHtml(aircraftDisplayType(plane, kind)) + '</strong>' + (operator ? '<br>Operator: ' + escapeHtml(operator) : '') + '</div>' + (plane.fireCallsign && plane.flight && plane.fireCallsign !== plane.flight ? '<div class="cw-popup-description">Transmitted callsign: <strong>' + escapeHtml(plane.flight) + '</strong></div>' : '') + fireInfo + '<div class="cw-popup-description">Altitude: <strong>' + escapeHtml(plane.altitude === null ? 'Unknown' : plane.altitude + ' ft') + '</strong><br>Ground speed: <strong>' + escapeHtml(plane.speed === null ? 'Unknown' : plane.speed + ' kt') + '</strong><br>Track: <strong>' + escapeHtml(plane.track === null ? 'Unknown' : plane.track + '°') + '</strong><br>Seen: ' + escapeHtml(plane.seen) + ' sec ago</div></div>';
      var aircraftContent = phoneMapPopup() ? '<strong>' + escapeHtml(label) + '</strong><br>' + escapeHtml(aircraftDisplayType(plane, kind)) + '<br>Registration: ' + escapeHtml(plane.registration || 'Unknown') + '<br>Altitude: ' + escapeHtml(plane.altitude === null ? 'Unknown' : plane.altitude + ' ft') + '<br>Speed: ' + escapeHtml(plane.speed === null ? 'Unknown' : plane.speed + ' kt') : aircraftPopup;
      L.marker([plane.latitude, plane.longitude], {icon: icon, zIndexOffset: plane.fireAircraft ? 650 : 500}).addTo(layerGroups.aircraft).bindPopup(aircraftContent, mapPopupOptions(390));
    });
  ...
  function aircraftDisplayType(plane, kind) {
    var model = String(plane.aircraftModel || plane.fireModel || '').trim();
    if (model) {
      var manufacturer = String(plane.aircraftManufacturer || plane.fireManufacturer || '').trim();
