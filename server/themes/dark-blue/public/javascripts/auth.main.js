angular.module('app', ['ngRoute', 'ngResource', 'ngSanitize', 'angular-uuid', 'ui.bootstrap', 'ui.validate', 'textAngular'])
    ...
    .controller('TwoFactorController', ['$scope', 'Api', '$window', function($scope, Api, $window) {
        $scope.verify = function() { $scope.loading = true; $scope.message = ''; Api.TwoFactor.verify({}, $scope.twoFactor).$promise.then(function(response) { $window.location.href = response.redirect; }, function(response) { $scope.loading = false; $scope.message = response.data.error || 'Verification failed.'; }); };
    }])
