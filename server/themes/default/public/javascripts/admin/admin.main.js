angular.module('app', ['ngRoute', 'ngResource', 'ngSanitize', 'angular-uuid', 'ui.bootstrap', 'color.picker', 'ui.validate', 'textAngular', 'ngFileSaver', 'angular-sortable-view'])
    ...
    .controller('UserDetailController', ['$scope', '$routeParams', 'Api', '$uibModal', '$filter', '$location', '$timeout', function ($scope, $routeParams, Api, $uibModal, $filter, $location, $timeout) {
      ...
      $scope.twoFactorReset = function() {
        if (!window.confirm('Reset two-factor authentication for this administrator? All trusted devices and recovery codes will be revoked.')) return;
        Api.UserTwoFactorReset.reset({id: $routeParams.id}, {}).$promise.then(function() {
          $scope.user.totp_enabled = false;
          $scope.alertMessage = {show:true, type:'alert-success', text:'Two-factor authentication reset.'};
        }, function(response) { $scope.alertMessage = {show:true, type:'alert-danger', text:response.data.error || 'Unable to reset two-factor authentication.'}; });
      };
